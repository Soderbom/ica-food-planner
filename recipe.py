...
class Recipe:
    def __init__(self, recipe_json):
        ...
        self.ingredients = self.parse_ingredients(
                recipe_json["IngredientsGroup"]
                )
        self.nutrition_per_portion = recipe_json["NutritionPerPortion"]
        self.cooking_steps = self.parse_cooking_steps(
                recipe_json["CookingSteps"])
        self.cooking_time = recipe_json["CookingTime"]
        self.rating = recipe_json["AverageRating"]

    def parse_ingredients(self, ingredients_group):
        ingredients = {}
        for ingredient in ingredients_group["Ingredients"]:
    ...
    def parse_cooking_steps(self, html_cooking_steps):
        cooking_steps = []
